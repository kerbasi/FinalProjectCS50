...
def params():
    if request.method == "POST":
        ...
        tanks = db.execute(
            "SELECT name, lvl, nation, is_premium FROM tanks ORDER BY lvl")
...
def load():
    ...
    radios = []
    for e in radios_id:
        radios.append(db.execute(
            "SELECT * FROM radios WHERE module_id = :module_id", module_id=e)[0])
    guns = []
    for e in guns_id:
        guns.append(db.execute(
            "SELECT * FROM guns WHERE module_id = :module_id", module_id=e)[0])
    suspensions = []
    for e in suspensions_id:
        suspensions.append(db.execute(
            "SELECT * FROM suspensions WHERE module_id = :module_id", module_id=e)[0])
    turrets = []
    if len(turrets_id[0]) > 0:
        for e in turrets_id:
            turrets.append(db.execute(
                "SELECT * FROM turrets WHERE module_id = :module_id", module_id=e)[0])
    else:
        turrets.append(db.execute(
            "SELECT * FROM turretsWOID WHERE id_tank = :id_tank", id_tank=answer[0]['id_tank'])[0])
    answer[0]['crew'] = crew
...
def getprofile():
    ...
    profile_id = ""
    profiles_var = db.execute(
        "SELECT profile_id FROM profile_ids WHERE tank_id = :tank_id", tank_id=tank_id)
